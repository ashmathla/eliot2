...
from confidence_engine import evaluate_signal_confidence
...
def extract_and_save_signals(
    ...
    liquidity: قاموس {"XAUUSD": {...}, "XAGUSD": {...}}
    من mt5_handler.detect_liquidity_proximity() لكل رمز.

    بدل الرفض المطلق لأي إشارة قرب القاع/القمة، تُقيَّم
    كل إشارة عبر confidence_engine.evaluate_signal_confidence()
    التي تُصدر قراراً متدرجاً:
        ALLOW  → تمر عادي
        WARN   → تمر مع تسجيل تحذير في notes
        REJECT → تُرفض فقط عند اجتماع قرب حقيقي +
                 Divergence مضاد + حجم تداول صاعد

    يُعيد عدد الإشارات المحفوظة.
    """
