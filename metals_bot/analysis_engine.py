...
def _format_symbol_section(
    ...
    # فيبوناتشي من H4 (للصورة الكبيرة) و H1 (للدخول الدقيق)
    ...
    return (
        ...
        f"{stoch_line}"
        f"{vwap_line}"
        f"     دعم: {h1.get('support')} "
        f"| مقاومة: {h1.get('resistance')}\n"
        f"M15: {m15.get('trend')}\n"
        f"FVG:\n{fvg}\n"
        f"OB + BOS:\n{h1.get('order_blocks')}\n"
        f"فيبوناتشي H4 (صورة كبيرة):\n{fib_h4}\n"
...
6. استخدم مستويات فيبوناتشي H1 كمرجع للدخول والأهداف:
   ...
   - إذا لم تتوافق الفيبوناتشي، اعتمد OB أو FVG بدلاً منها
     واذكر السبب — لا تُصدر WAIT بسببها وحدها.
...
def analyze_metals_with_memory():
    ...
    # بيانات السيولة لكل رمز — تُمرَّر كحارس برمجي
    # صارم في extract_and_save_signals، بحيث تُرفض أي
    # إشارة SELL قرب القاع أو BUY قرب القمة حتى لو
    # تجاهل Gemini التحذير النصي في الـ Prompt.
    liquidity_context = {
        "XAUUSD": summaries["gold_h1"].get("liquidity", {}),
        "XAGUSD": summaries["silver_h1"].get("liquidity", {}),
    }
    saved = extract_and_save_signals(
        response_text, liquidity_context
    )
